        ...
        <div class="flex-grow">
          <vs-dropdown vs-trigger-click class="cursor-pointer">
            <div class="p-4 border border-solid d-theme-border-grey-light rounded-full d-theme-dark-bg cursor-pointer flex items-center justify-between font-medium">
              <span class="mr-2">{{ currentPage * paginationPageSize - (paginationPageSize - 1) }} - {{ companiesData.length - currentPage * paginationPageSize > 0 ? currentPage * paginationPageSize : companiesData.length }} {{ $t('of') }} {{ companiesData.length }}</span>
...
export default {
  ...
  computed: {
    companiesData () {
      ...
      return  _.map(this.$store.state.companyManagement.companies, (company) => {
        company.status = company.deleted_at === null ? 'active' : 'deactivated'
        return company
      })
    },
    paginationPageSize () {
