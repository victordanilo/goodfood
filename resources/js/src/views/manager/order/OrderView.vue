...
<template>
  <div id="page-user-view">
    <vs-alert color="danger" :title="title_order_not_found" :active.sync="order_not_found">
      ...
      <span>
        <span>{{ $t('check') }} </span><router-link :to="{name:'manager-orders'}" class="text-inherit underline">{{ $t('all_orders') }}</router-link>
      </span>
    </vs-alert>
        ...
        <vx-card :title="title_order" class="mb-base">
          <template slot="actions">
            <div class="vx-card__code-toggler sm:block hidden">
              <vs-button color="dark" type="flat" :to="{name: 'manager-orders'}">{{$t('back')}}</vs-button>
            </div>
          </template>
...
<script>
import moduleOrderManagement from '@/store/manager/order-management/moduleOrderManagement'
...
export default {
  ...
  created () {
    ...
    this.$store.dispatch('orderManagement/fetchOrder', orderUUID)
      ...
      .catch(err => {
        if (err === 'order_not_found' || err.response.status === 404) {
