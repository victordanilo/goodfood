...
<template>
  <div id="order-view" class="container">
    <vs-alert color="danger" :title="title_order_not_found" :active.sync="order_not_found">
      ...
      <span>
        <span>{{ $t('check') }} </span><router-link :to="{name:'orders'}" class="text-inherit underline">{{ $t('all_orders') }}</router-link>
      </span>
    </vs-alert>
        ...
        <vx-card :title="title_order" class="mb-base">
          <template slot="actions">
            <div class="vx-card__code-toggler sm:block hidden">
              <vs-button color="dark" type="flat" :to="{name: 'orders'}">{{$t('back')}}</vs-button>
            </div>
          </template>
